...
for i = 1:length(exerciseRLocs)
    ...
    plot(0:200, heartbeatSegment, 'Color', [1 0 0 0.5]); % Red
end
...
for i = 1:length(restingRLocs)
    ...
    plot(0:200, heartbeatSegment, 'Color', [0 0 1 0.5]); % Blue
end
...
for i = 1:length(boxBreathingRLocs)
    ...
    plot(0:200, heartbeatSegment, 'Color', [0 1 0 0.5]); % Green
end
...
% %% Step 8 mới: Compute the derivative and speed of Resting R & T pulses
% 
% % Find the T peaks from Resting Data 
% % find all the peaks (again)
% minPeakHeightRestingT = 0.15;
% maxPeakHeightRestingT = 0.4;  
% [allPeaks, allLocs] = findpeaks(RestingData(:, 2), RestingData(:, 1), 'MinPeakHeight',minPeakHeightRestingT);
% restingTpeaks = cat(2,allLocs, allPeaks);       % restingTpeaks(:, 1) is time , restingTpeaks(:, 2) is voltage
% 
% 
% % Get only the T-peaks and not the others 
% restingTpeaks = restingTpeaks(restingTpeaks(:,2) <= maxPeakHeightRestingT, :);
% 
% % %   Plot to check T-peaks are selected correctly, comment out afterwards
% % figure
% % plot(RestingData(:, 1), RestingData(:, 2))
% % hold on
% % plot(restingTpeaks(:, 1), restingTpeaks(:, 2), 'v', 'MarkerFaceColor','b', 'MarkerEdgeColor','b')
% % hold off
% % xlim([120 130])
% 
% % Put all the R peaks data into one variable 
% restingRpeaks_together = cat(2,restingRLocs, restingRPeaks);    % restingRpeaks_together(:, 1) is time , restingTpeaks_together(:, 2) is voltage
% % put time and voltage column from resting data together into one variable 
% resting_data_new = cat(2,RestingData(:,1), RestingData(:,2));
% 
% % 10 samples before and after each R and T pulse
% nsample = 10; 
% 
% % Get 10 samples before each R and before T peaks
% before_R_delta_voltage = zeros(length(restingRpeaks_together),1);
% for i = 1:length(restingRpeaks_together)
%     peak_id = find(resting_data_new(:,2) == restingRpeaks_together(i,2) & resting_data_new(:,1) == restingRpeaks_together(i,1));       %find peak_id in the resting_data vector 
%     r_before = peak_id - nsample: peak_id - 1;
%     before_R = resting_data_new(r_before, :);
%     m = mean( diff(before_R(:,2))/diff(before_R(:,1)) );
%     before_R_delta_voltage(i) = m(1);
% 
% end
% 
% before_T_delta_voltage = zeros(length(restingTPeaks),1);
% for i = 1:length(restingTPeaks)
%     peak_id = find(resting_data_new(:,2) == restingTPeaks(i,1) & resting_data_new(:,1) == restingTPeaks(i,2));       %find peak_id in the resting_data vector 
%     %restingTPeaks(:,1) is voltage, restingTPeaks(:,2) is time, 
%     t_before = peak_id - nsample: peak_id - 1;
%     before_T = resting_data_new(t_before, :);
%     m= mean( diff(before_T(:,2))/diff(before_T(:,1)) );
%     before_T_delta_voltage(i) = m(1);
% 
% end
% 
% % Get 10 samples after R and after T peaks
% 
% after_R_delta_voltage = zeros(length(restingRpeaks_together),1);
% for i = 1:length(restingRpeaks_together)
%     peak_id = find(resting_data_new(:,2) == restingRpeaks_together(i,2) & resting_data_new(:,1) == restingRpeaks_together(i,1));       %find peak_id in the resting_data vector 
%     r_after = peak_id + 1:peak_id + nsample;
%     after_R = resting_data_new(r_after, :);
%     m = mean(diff(after_R(:,2))/diff(after_R(:,1)));
%     after_R_delta_voltage(i) = m(1);
% 
% end
% 
% after_T_delta_voltage = zeros(length(restingTPeaks),1);
% for i = 1:length(restingTPeaks)
%     peak_id = find(resting_data_new(:,2) == restingTPeaks(i,1) & resting_data_new(:,1) == restingTPeaks(i,2));       %find peak_id in the resting_data vector 
%     %restingTPeaks(:,1) is voltage, restingTPeaks(:,2) is time, 
%     t_after = peak_id + 1:peak_id + nsample;
%     after_T = resting_data_new(t_after, :);
%     m = mean( diff(after_T(:,2))/diff(after_T(:,1)) );
%     after_T_delta_voltage(i) = m(1);
% end
% 
% % Generate histogram 
% % take absolute values of the after data 
% after_T_delta_voltage = abs(after_T_delta_voltage); 
% after_R_delta_voltage = abs(after_R_delta_voltage);
% 
% %Combine the R before and R after into one 
% R_pulse_speed = [before_R_delta_voltage; after_R_delta_voltage]; 
% %Combine the T before and T after into one 
% T_pulse_speed = [before_T_delta_voltage; after_T_delta_voltage]; 
% 
% %generate histogram 
% figure;
% histogram(R_pulse_speed, 'BinWidth', .1, 'Normalization', 'probability');
% hold on;
% histogram(T_pulse_speed, 'BinWidth', .1, 'Normalization', 'probability');
% hold off;
% 
...
% % %restingTPeaks(:,1) is voltage, restingTPeaks(:,2) is time, 
% % t_before = peak_id - nsample: peak_id - 1;
% % %before_R(i:nsample) = resting_data_new(r_before, :); 
% % bf = resting_data_new(t_before, :);
% % % plot to check
% % plot(resting_data_new(:,1), resting_data_new(:,2), 'LineWidth',1)
% % hold on 
% % plot(bf(:, 1),bf(:, 2), '*')
% % hold off 
% % xlim([81 85])
...
hold off;
xlabel('voltage change (mV/s)', 'FontSize', 12);
ylabel('Probability', 'FontSize', 12);
title('Rate of change of voltage for different phases', 'FontSize', 14);
...
%Run a t-test on the heart rate frequency during two separate periods of the rest data
% 1:10 is from 150 sec to 158 sec period 
% 20:30 is from 165 sec to 173 sec period 
x2 = restingHeartRate(1:10);
y2 = restingHeartRate(20:30);

result_sep_period = ttest2(x2,y2);
disp(result_sep_period)
if result_sep_period == 1
    disp('The null hypothesis is resting heart rate is there is no difference among different time period. We reject the null hypothesis.')
elseif result_sep_period == 0 
    disp('The null hypothesis is resting heart rate is there is no difference among different time period. We cannot reject the null hypothesis.')
end   
