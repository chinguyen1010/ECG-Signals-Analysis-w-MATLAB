...
The first column is the time vector in s, the second column is the recorded ECG in mV, and the last column is a calculation of the instantaneous heart rate based on the ECG. NOTE: This calculation is done by the software and may not be accurate, so we’re not going to use it!
